...
def launch_gui():
    ...
    options_layout = [
        ...
        [sg.Text('Pitch (μm):'), sg.Input(key='-PITCH-', size = 6)],
        [sg.Text('Min radius (μm):'), sg.Input(key='-MINR-', size = 6), sg.Text('Max radius (μm):'), sg.Input(key='-MAXR-', size = 6)],
        
    ]
    
    
    layout = [
            [sg.Text('AFM Data Analysis Tool', size=(38, 1), justification='center', font=("Helvetica", 16), relief=sg.RELIEF_RIDGE, k='-TEXT HEADING-', enable_events=True)],
    ...
    return filename_list, show_all, minr, maxr
