...
4. User interface pops up. Click the `Browse` button. 
5. File Explorer window pops up. Drag and select files to be analyzed, and click `Open`. 
6. Click the `Options` tab to customize output. See `Additional Functions` section below for more details. 
7. After selecting additional functions needed, go back to the main tab and click `Submit` button. 
8. Scripts starts to process files. You can see in the progress bar how many files it has processed. Time needed is approximately 14s per file. 
9. If you want to terminate the process halfway, press CTRL+C in the terminal.
10. After all scripts has been processed, you can find the Excel file generated by clicking into the `AFMTOOL` folder in your Desktop, click into  `results` folder and then into `xlSheets` folder. The Excel file name is of the timestamped format `DDMMYYYYHHMM_AFMResults`, where the timestamp is the time when you started running the script (after you click `Open` in step 4). 
...
https://user-images.githubusercontent.com/105037297/171802895-f6072c85-b589-4633-b713-4515d207a85c.mp4
